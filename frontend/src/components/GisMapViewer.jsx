  ...
  if (isEmergency) {
    ...
    innerHtml = `
      <div class="relative flex items-center justify-center w-full h-full">
        <span class="absolute w-full h-full rounded-full bg-red-500/20 ${isSelected ? 'ring-2 ring-red-400/60' : ''}"></span>
        <span class="relative w-2.5 h-2.5 rounded-full bg-red-500 border border-white shadow-sm"></span>
      </div>
    `;
  } else if (isFlare) {
    ...
    innerHtml = `
      <div class="relative flex items-center justify-center w-full h-full">
        ${isSelected ? '<span class="absolute w-full h-full rounded-full bg-orange-400/20 ring-1 ring-orange-300"></span>' : ''}
        <span class="relative w-2 h-2 rounded-full bg-orange-400 border border-slate-900"></span>
      </div>
    `;
  } else if (isCoal) {
    ...
    innerHtml = `
      <div class="relative flex items-center justify-center w-full h-full">
        <span class="relative w-1.5 h-1.5 rounded-full bg-amber-400 border border-slate-900"></span>
      </div>
    `;
  } else {
    // Stubble or Forest fire
    size = isSelected ? 15 : 11;
    const color = fire.category === 'FOREST_FIRE' ? '#10b981' : '#22c55e';
    innerHtml = `
      <div class="relative flex items-center justify-center w-full h-full">
        <span class="w-1.5 h-1.5 rounded-full" style="background-color: ${color}; opacity: 0.85;"></span>
      </div>
    `;
  }
      ...
      <div className="absolute bottom-4 left-4 z-[999] bg-[#0c1015]/90 border border-white/[0.08] rounded-md p-2.5 text-[11px] shadow-lg backdrop-blur-md text-slate-300 select-none font-sans">
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2 pt-0.5 border-t border-white/[0.08] mt-1">
            ...
            <span className="text-sky-300 font-mono text-[10px]">OSM Industrial Perimeter</span>
          </div>
        </div>
      </div>
    </div>
